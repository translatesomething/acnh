...
function fetchItemBatch(endpoint, names) {
  ...
  return fetchWithConcurrency(tasks, 3).then(r => r.filter(Boolean));
}
...
export async function getVillagers() {
  try {
    const res = await fetch(`${API_URL}/villagers?nhdetails=true`, { headers: HEADERS });
...
export async function getVillagerDetails(name) {
  try {
    const res = await fetch(`${API_URL}/villagers?name=${encodeURIComponent(name)}&nhdetails=true`, { headers: HEADERS });
...
export async function getCritters(type) {
  try {
    const res = await fetch(`${API_URL}/nh/${type}`, { headers: HEADERS });
...
export async function getEvents() {
  try {
    const res = await fetch(`${API_URL}/nh/events`, { headers: HEADERS });
...
export async function getArt() {
  try {
    const res = await fetch(`${API_URL}/nh/art`, { headers: HEADERS });
...
export async function getFossils() {
  try {
    const [individuals, groups] = await Promise.all([
      fetch(`${API_URL}/nh/fossils/individuals`, { headers: HEADERS }).then(r => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
      fetch(`${API_URL}/nh/fossils/groups`, { headers: HEADERS }).then(r => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
    ]);
...
export async function getGyroids() {
  try {
    const res = await fetch(`${API_URL}/nh/gyroids`, { headers: HEADERS });
