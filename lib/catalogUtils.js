...
export function getPageNumbers(currentPage, totalPages) {
  ...
  if (totalPages <= 7) { for (let i = 1; i <= totalPages; i++) pages.push(i); }
    ...
    else { pages.push('...'); for (let i = currentPage - 1; i <= currentPage + 1; i++) pages.push(i); pages.push('..x'); pages.push(totalPages); }
  }
  return pages;
}
...
export function bgLoad({ names, cached, batchFn, batchSize = 5, delay = 500, controller, onProgress, onDone }) {
